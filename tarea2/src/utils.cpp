...
string Cmd_parser::get_opt(const string& flag){
   string value = "";
...
vector<string> Cmd_parser::get_multiple_opt(const string& flag){
...
bool Cmd_parser::opt_exists(const string& flag){
   auto itr = find(input_vec.begin(), input_vec.end(), flag);
...
Cmd_params::Cmd_params(int& argc, char **argv)
   : cmd_parse(argc,argv) {
